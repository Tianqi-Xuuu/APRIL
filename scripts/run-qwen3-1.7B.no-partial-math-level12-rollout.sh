...
RUN_NAME=${RUN_NAME:-qwen3-1.7b-no-partial-math-level12-bs64-n1}
RUN_ROOT=${RUN_ROOT:-/root/APRIL/runs/${RUN_NAME}}
INPUT_DATA=${INPUT_DATA:-/root/math_level12/data/math-level12-train.parquet}
...
CKPT_ARGS=(
  ...
  --load /root/.slime-nonexistent-qwen3-1.7B-no-partial-math-level12-load
  --save "${RUN_ROOT}"
)
