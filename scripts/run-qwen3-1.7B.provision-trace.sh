...
RUN_ROOT_BASE=${RUN_ROOT_BASE:-/root/APRIL/runs/provision-trace-qwen3-1.7b-bs${ROLLOUT_BATCH_SIZE}-len${ROLLOUT_MAX_RESPONSE_LEN}}
...
COMMON_CKPT_ARGS=(
   ...
   --load /root/.slime-nonexistent-qwen3-1.7B-provision-trace-load
)
...
python /root/APRIL/scripts/analysis/plot_decode_throughput_trace.py \
    --run-root-base "${RUN_ROOT_BASE}" \
    --output-csv "${RUN_ROOT_BASE}/decode_throughput_trace.csv" \
    --output-plot "${RUN_ROOT_BASE}/decode_throughput_trace.png"
